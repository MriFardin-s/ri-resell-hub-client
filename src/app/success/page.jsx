...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    const fetchSessionDetails = async () => {
      try {
        const res = await fetch(`/api/checkout_sessions/retrieve?session_id=${sessionId}`);
        ...
        <div className="bg-neutral-50 dark:bg-neutral-900/40 border border-neutral-200/60 dark:border-neutral-700 p-5 rounded-2xl mb-8 text-left space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="bg-green-500/10 text-green-600 dark:text-green-400 px-2.5 py-0.5 rounded-lg font-bold uppercase text-[10px] tracking-wider border border-green-500/20">
              {orderData?.paymentStatus === 'paid' ? 'Paid' : orderData?.paymentStatus}
            </span>
          </div>

          <div className="border-t border-neutral-200 dark:border-neutral-700/60 pt-3">
            <span className="text-xs text-neutral-400 font-medium block mb-1.5">Receipt Sent To</span>
